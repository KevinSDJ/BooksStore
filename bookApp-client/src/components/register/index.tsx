...
const Register = (): JSX.Element => {
  ...
  const [isFetch,setIsFetch] = useState<Boolean|null>(false)

  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setIsFetch(!isFetch)
    authService.signUp(formvalue,(data:any)=>{
      console.log(data)
    })
  };
  ...
  return (
      ...
      className=" lg:w-110 flex flex-col justify-center self-center px-3 py-6 rounded-3xl bg-white">
        <div className="px-6 my-4 bg-white">
          <div className="mb-4 pt-4">
            <h3 className="font-semibold text-2xl text-gray-800">Sign Up </h3>
            <p className="text-gray-400 text-sm">You already account? <Link to="/auth"
              className=" text-purple-500 hover:text-purple-700">Sign In</Link></p>
          </div>
          <div className="space-y-4 w-full">
            <div className="space-y-1 w-full">
              <label htmlFor='username' className="text-gray-400 font-semibold my-5" >Username</label>
              <input 
            ...
            <div className="space-y-1 w-full">
              <label htmlFor='email'  className="text-gray-400 font-semibold my-5" >Email</label>
              <input 
              className=" w-full text-sm  px-4 py-3 bg-gray-200 focus:bg-gray-100 border  border-gray-200 rounded-lg focus:outline-none focus:border-purple-400" 
              type="email"
              onChange={handleChange}
            ...
            <div className="relative space-y-1" x-data="{ show: true }">
              <label htmlFor='password'  className="text-gray-400 font-semibold my-5" >Password</label>
              <input 
        ...
        <div className='px-6'>
          ...
          className="disabled:opacity-30 w-full flex justify-center bg-purple-800  hover:bg-purple-600 text-gray-100 p-3  rounded-lg tracking-wide font-semibold  cursor-pointer transition ease-in duration-500">
            Sign in
          </button>
        </div>
